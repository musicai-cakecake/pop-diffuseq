...
def helper_tokenize(sentence_lst, vocab_dict, seq_len):
    ...
    print(raw_datasets)
    print(f"RAM datasets used: {psutil.Process().memory_info().rss / (1024 * 1024):.2f} MB")
    ...
    def merge_and_mask(group_lst):
        lst = []
        mask = []  # 每条list里是一共src的长度加1个0

        # list_len = int((seq_len - attribute_num)/2 - 2*attribute_num)  # 90

        for i in range(len(group_lst['input_id_x'])):  # chord = 4
            end_token = group_lst['input_id_x'][i][-1]  # end_token = 1

            src = group_lst['input_id_x'][i][:8]  # list
            trg = group_lst['input_id_y'][i][:501]  # list
        ...
        # print("group_lst['input_ids'][0]=", group_lst['input_ids'][0])
        # print("group_lst['input_mask'][0]=", group_lst['input_mask'][0])
        return group_lst
    ...
    def pad_function(group_lst):
        ...
        # 把和弦等不需要AI预测的控制变量设置为 0， 把需要预测的置为1

        for i in range(len(group_lst['input_ids'])):
            for j in range(len(group_lst['input_ids'][i])):
                if group_lst['input_ids'][i][j] in [4, 5]:
                    group_lst['input_mask'][i][j] = int(0)
                    group_lst['input_mask'][i][j + 1] = int(0)
                    group_lst['input_mask'][i][j + 2] = int(0)
                    group_lst['input_mask'][i][j + 3] = int(0)
                elif group_lst['input_ids'][i][j] in [6, 24]:
                    group_lst['input_mask'][i][j] = int(0)
