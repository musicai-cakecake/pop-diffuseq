...
from typing import List, Tuple, Dict, Union, Generator, Collection
...
class Vocabulary:
    ...
    def numericalize(self, t: Collection[str]) -> List[int]:
        """Convert a list of words `t` to their ids."""
        return [self._event_to_token[w] for w in t]

    def textify(self, nums: Collection[int], sep=' ') -> List[str]:
        """Convert a list of `nums` to human readable words."""
        items = [self._token_to_event[i] for i in nums]
    ...
    def token_value(self, token: int) -> str:
        """Returns the value of the given token

        :param token: token to get type from
        :return: the type of the token, as a string
        """
        return self._token_to_event[token].split('_')[1]
    ...
    def event_to_token(self):
        return self._event_to_token  # words2idx

    @property
    def token_to_event(self):
        return self._token_to_event  # idx2words
